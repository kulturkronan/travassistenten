...
async function generateV75StartlistMarkdown(game, dateStr) {
  ...
  for (let i = 0; i < races.length; i++) {
    ...
    for (const st of starts) {
      ...
      if (lastFiveRecords.length > 0) {
        for (const record of lastFiveRecords) {
          ...
          // Bana (om tillgängligt) - försök extrahera från code
          let track = "-";
          if (record.code) {
            // Code verkar innehålla information om lopptyp
            // t.ex. "aM" = auto medium, "M" = volte medium
            track = record.code;
          }
    ...
    const raceId = r?.id;
    if (raceId) {
      const tipsData = await fetchRaceTips(raceId);
