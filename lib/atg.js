...
function generateV75ResultMarkdown(game, dateStr) {
  ...
  for (const r of races) {
      ...
      .map(st => {
        ...
        if (timeValue) {
          if (typeof timeValue === "object") {
            ...
            time = `${minutes}.${seconds.toString().padStart(2, '0')},${tenths}`;
          } else {
            time = s(timeValue);
          }
        }
        ...
        const v75Percent = v75Distribution > 0 ? (v75Distribution / 100).toFixed(1) + "%" : "";
    ...
    for (const row of withResult) {
      ...
      md += `| ${row.place ?? ""} | ${s(row.number)} | ${s(row.horse)} | ${s(row.driver)} | ${s(row.v75Percent)} | ${s(row.time)} | ${status} |\n`;
    }
...
function generateV75StartlistMarkdown(game, dateStr) {
  ...
  for (const r of races) {
    ...
    md +=
      "| Nr | Häst | Kusk | Tränare | Spår | Hcp | Form | Rekord | Vinstsumma | V75% | V | P |\n" +
      "|---:|---|---|---|---:|---:|---|---:|---|---:|---:|---:|\n";
    ...
    for (const st of starts) {
      ...
      if (st?.horse?.record) {
        if (typeof st.horse.record === "object") {
          ...
          if (time && typeof time === "object") {
            ...
            record = `${minutes}.${seconds.toString().padStart(2, '0')},${tenths}`;
          } else {
      ...
      // Win and place odds
      const winOdds = st?.pools?.vinnare?.odds ? (st.pools.vinnare.odds / 100).toFixed(2) : "";
      const placeOdds = st?.pools?.plats?.minOdds ? (st.pools.plats.minOdds / 100).toFixed(2) : "";

      // Formatera vinstsumma med mellanslag
      const earnings = st?.horse?.money ?? st?.horse?.earnings ?? 0;
      const formattedEarnings = earnings > 0 ? earnings.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") : "";

      md += `| ${s(st?.number)} | ${s(st?.horse?.name)} | ${s(st?.driver?.name || st?.driver?.shortName)} | ${s(st?.trainer?.name)} | ${s(st?.postPosition ?? st?.startNumber ?? st?.number)} | ${s(st?.handicap ?? "")} | ${s(st?.horse?.form ?? "")} | ${record} | ${formattedEarnings} | ${v75Percent} | ${winOdds} | ${placeOdds} |\n`;
    }
    ...
    if (tips && Array.isArray(tips) && tips.length > 0) {
      ...
      for (const tip of tips) {
        ...
        if (horses.length > 0) {
          const horseList = horses.map(h => `${h?.number || ""}. ${h?.name || h?.horse || ""}`).join(", ");
          md += `- **${s(type)}:** ${horseList}\n`;
        } else if (description) {
