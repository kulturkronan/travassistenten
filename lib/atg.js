  ...
  for (let i = 0; i < races.length; i++) {
    ...
    for (const st of starts) {
      ...
      if (tipsData && tipsData.comments) {
        const horseComment = tipsData.comments.find(c => c.startNumber === st?.number);
        if (horseComment) {
      ...
      md += `| ${s(st?.number)} | ${s(st?.horse?.name)} | ${s(
        ...
        st?.horse?.form ?? ""
      )} | ${record} | ${formattedEarnings} | ${v75Percent} | ${trendPercent} | ${winOdds} | ${placeOdds} | ${shoes} | ${wagon} | ${s(horseTip)} |\n`;
      ...
      if (lastFiveRecords.length > 0) {
        for (const record of lastFiveRecords) {
          ...
          // Bana (om tillgängligt) - konvertera kod till mer läsbar format
          let track = "-";
          if (record.code) {
            // Mappning av koder till mer läsbara namn
            const trackMap = {
              'aM': 'Auto-Medium',
              'aL': 'Auto-Long', 
              'aK': 'Auto-Short',
            ...
            track = trackMap[record.code] || record.code;
          }
