...
/**
 * Hämta detaljerad hästinformation via häst-ID
 */
...
/**
 * Hämta hästhistorik för alla hästar i en V75-omgång via häst-ID:er
 */
...
function extractHistoryFromHorseDetails(horseDetails) {
  ...
  // Hämta rekord från statistics.life.records
  const records = horseDetails?.statistics?.life?.records || [];

  for (const record of records) {
    const time = record?.time || {};
    const kmTime =
      time.minutes && time.seconds && time.tenths
        ? `${time.minutes}.${time.seconds.toString().padStart(2, "0")},${
            time.tenths
          }`
        : "";

    // Konvertera distans från "short", "medium", "long" till meter
    let distance = "";
    if (record?.distance === "short") distance = "2140";
    else if (record?.distance === "medium") distance = "2640";
    else if (record?.distance === "long") distance = "3140";
    else distance = record?.distance || "";
