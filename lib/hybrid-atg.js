...
async function scrapeHorseHistory(raceUrl) {
  ...
  try {
    ...
    try {
      ...
      // Klicka på "Lösenord" i login modal
      await page.click('button:has-text("Lösenord")', { timeout: 10000 });
      await page.waitForTimeout(2000);

      // Fyll i användarnamn
      await page.fill('input[type="text"]', "jesSjo680", { timeout: 10000 });
      await page.waitForTimeout(1000);

      // Fyll i lösenord
      await page.fill('input[type="password"]', "Jeppe1599", {
        timeout: 10000,
      });
      await page.waitForTimeout(1000);
...
async function generateHybridV75StartlistMarkdown(game, dateStr) {
  ...
  for (let i = 0; i < races.length; i++) {
    ...
    md +=
      "| Nr | Häst/Kusk | V75% | TREND% | V-ODDS | Tränare | Tipskommentar | Skor | Vagn |\n";
    md += "|---:|---|---|---:|---:|---|---|---:|---:|---:|---:|---:|---|\n";
    ...
    for (const st of starts) {
      ...
      const horseKusk = `${horseName} / ${driver}`;
      const trainer = s(st?.trainer?.name);

      // Använd formatfunktioner för att hantera objekt korrekt
      const shoes = formatShoes(st?.shoes);
      const wagon = formatWagon(st?.sulky);
      ...
      // TIPSKOMMENTAR - kommer från inloggning
      const tipskommentar = "";

      md += `| ${number} | ${horseKusk} | ${v75Percent}% | ${trendPercent} | ${vinnareOdds} | ${trainer} | ${tipskommentar} | ${shoes} | ${wagon} |\n`;
    }
...
async function generateHistoricalDataMarkdown(game, dateStr) {
  ...
  // Hämta hästhistorik via API (effektiv metod)
  console.log(
    `🔍 Hämtar hästhistorik via API för ${races.length} avdelningar...`
  );
  const horseHistory = await getV75HorseHistory(game.id);
  ...
  for (let i = 0; i < races.length; i++) {
    ...
    for (const st of starts) {
      ...
      if (horseHistory[startNumber] && horseHistory[startNumber].length > 0) {
        ...
        md += "*Hästhistorik kunde inte extraheras via API.*\n";
      }
