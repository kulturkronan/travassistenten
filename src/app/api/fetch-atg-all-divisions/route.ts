...
export async function POST(request: NextRequest) {
  console.log("🚀 Hämtar alla V75-avdelningar från ATG...");
  ...
  try {
    ...
    for (let i = 1; i <= 7; i++) {
      try {
        ...
        // Hämta data direkt istället för att anropa intern API
        ...
        if (raceData && raceData.length > 0) {
          ...
          const updatedRaceData = raceData.map((race) => ({
            ...race,
            raceNumber: i, // Använd loop-indexet som raceNumber
            title: `V75-${i} - ${race.title.split(" - ")[1] || "Bjerke"}`, // Uppdatera titel
          }));
