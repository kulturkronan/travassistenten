import { V75Data, DivisionData, HorseData } from "./schemas.js";
import fs from "fs";
...
export class MarkdownWriter {
  ...
  constructor(outputPath: string = "V75_Startlista.md") {
  ...
  private generateMarkdown(data: V75Data): string {
  ...
  private generateDivisionMarkdown(division: DivisionData): string {
    ...
    for (const horse of division.horses) {
      markdown += `| ${horse.number} | ${horse.name} | ${horse.driver} | ${
        horse.trainer
      } | ${horse.v75Percent || "N/A"} | ${horse.trend || "N/A"} | ${
        horse.odds || "N/A"
      } | ${horse.shoes || "N/A"} |\n`;
    }
  ...
  private generateHorseDetailsMarkdown(horse: HorseData): string {
  ...
  private generateSpeltipsMarkdown(speltips: any): string {
  ...
  public createBackup(): string {
    ...
    const backupPath = this.outputPath.replace(
      ".md",
