...
interface DetailedRace {
  raceNumber: number;
  raceInfo: RaceInfo;
  horses: DetailedHorse[];
  ...
  try {
    ...
    await new Promise((resolve) => {
      process.stdin.once("data", () => {
        console.log(
          "✅ Användaren är redo att börja med förbättrad granskning!"
        );
        resolve(void 0);
      });
    });
    ...
    for (let avdelning = 1; avdelning <= 7; avdelning++) {
      ...
      console.log(
        "      📊 Loppinformation (titel, distans, prispengar, pool)"
      );
      ...
      try {
        ...
        const improvedData = await page.evaluate((raceNum) => {
          ...
          const distanceMatch = bodyText.match(
            /(\d+)\s+m\s+(Voltstart|Autostart)/
          );
          if (distanceMatch) {
          ...
          const horsePattern =
            /^(\d+)\s+([A-ZÅÄÖ][a-zåäö\s]+)\s+([a-z]\d+)\s+([A-ZÅÄÖ][a-zåäö\s]+)$/gm;
          let horseMatch;
          ...
          return {
            ...
            hasScratchedHorses:
              bodyText.includes("EJ") || bodyText.includes("struken"),
            scratchedCount,
            // Debug info
            debugInfo: {
        ...
        console.log(
          `   🐛 Debug: V75=${improvedData.debugInfo.hasV75Text}, Häst=${improvedData.debugInfo.hasHorseText}, Tips=${improvedData.debugInfo.hasTipsText}`
        );
        ...
        if (improvedData.bettingTips.length > 0) {
          ...
          improvedData.bettingTips.forEach((tip, index) => {
            console.log(
              `      ${index + 1}. ${tip.horseNumber} ${tip.horseName}: "${
                tip.tipText
              }"`
            );
          });
        ...
        const detailedRace: DetailedRace = {
        ...
        v75Data.syncInfo.totalHorses = v75Data.races.reduce(
        ...
        console.log(
          `📊 Kvalitetspoäng: ${v75Data.syncInfo.qualityScore.toFixed(1)}%`
        );
    ...
    console.log(
      `   ⏰ Senast synkroniserat: ${new Date(
        v75Data.syncInfo.lastSync
    ...
    v75Data.races.forEach((race) => {
      console.log(
        `   🏁 Avdelning ${race.raceNumber}: ${race.horses.length} hästar, ${race.bettingTips.length} tips, kvalitet: ${race.qualityCheck.dataQuality}`
      );
    });
    ...
    console.log(
      "💾 Final förbättrad data sparad som v75-improved-complete.json"
    );
