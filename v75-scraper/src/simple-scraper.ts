...
export async function scrapeV75Simple(): Promise<SimpleV75Data> {
...
function parseV75HTML(html: string): SimpleRace[] {
  const races: SimpleRace[] = [];
  ...
  for (let i = 0; i < Math.min(divisions.length, 7); i++) {
    ...
    const scratchedRegex =
      /häst\s+(\d+).*?(struken|avstängd|ej startar|startar inte)/gi;
    const scratchedMatches = [...html.matchAll(scratchedRegex)];

    const scratchedHorses = new Set<number>();
    ...
    console.log(
      `Avdelning ${raceNumber}: ${
        scratchedHorses.size
      } strukna hästar (${Array.from(scratchedHorses).join(", ")})`
    );
