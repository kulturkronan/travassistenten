...
interface DetailedHorse {
  ...
  tipsComment: string;
  scratched: boolean;
  ageGender?: string; // t.ex. "s4" för 4-årig sto
}
...
async function detailedV75Session() {
  ...
  if (fs.existsSync(dataFile)) {
    try {
      ...
      console.log(
        `📂 Laddade befintlig detaljerad data med ${v75Data.races.length} avdelningar`
      );
    } catch (error) {
      console.log(
        "📂 Ingen befintlig detaljerad data hittades, börjar från början"
      );
    }
  }
  ...
  try {
    ...
    await new Promise((resolve) => {
      process.stdin.once("data", () => {
        console.log(
          "✅ Användaren är redo att börja med detaljerad granskning!"
        );
        resolve(void 0);
      });
    });
    ...
    for (let avdelning = 1; avdelning <= 7; avdelning++) {
      ...
      try {
        ...
        const detailedData = await page.evaluate((raceNum) => {
          ...
          const horseRows = [];
          const horsePattern =
            /(\d+)\s+([A-ZÅÄÖ][a-zåäö\s]+)\s+([a-z]\d+)\s+([A-ZÅÄÖ][a-zåäö\s]+)/g;
          let horseMatch;
    ...
    console.log(
      "💾 Final detaljerad data sparad som v75-detailed-complete.json"
    );
