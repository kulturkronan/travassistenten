...
import { V75Data, CliArgs } from "./schemas.js";
...
class V75Scraper {
    ...
    async loginToATG(page) {
        try {
            ...
            await page.waitForSelector('input[name="username"], input[type="email"], input[placeholder*="användarnamn"]', { timeout: 10000 });
            // Fyll i användarnamn
            await page.fill('input[name="username"], input[type="email"], input[placeholder*="användarnamn"]', "jesSjo680");
            // Fyll i lösenord
            await page.fill('input[name="password"], input[type="password"]', "Jeppe1599");
            // Klicka på inloggningsknappen
            await page.click('button[type="submit"], button:has-text("Logga in"), input[type="submit"]');
            ...
            if (!cookieHandled) {
                ...
                try {
                    await page.evaluate(() => {
                        ...
                        const cookieButton = buttons.find((btn) => btn.textContent?.includes("Godkänn") ||
    ...
    async scrapeV75(date, track, bane) {
        if (!this.browser) {
        ...
        try {
            ...
            for (let avd = 1; avd <= 7; avd++) {
                ...
                const divisionData = await this.scrapeDivision(page, date, track, avd, bane);
    ...
    async scrapeDivision(page, date, track, divisionNumber, bane) {
        ...
        while (attempts < this.config.retryAttempts) {
            try {
                ...
                await page.waitForSelector('[data-test="startlist-row"], .StartListRow, .startlist-row, [class*="horse"], [class*="start"]', {
                    timeout: 15000,
                });
                ...
                attempts++;
                console.warn(`Försök ${attempts} misslyckades för avdelning ${divisionNumber}:`, error);
                if (attempts < this.config.retryAttempts) {
                    await page.waitForTimeout(2000);
                }
                else {
                    throw new Error(`Kunde inte scrapa avdelning ${divisionNumber} efter ${this.config.retryAttempts} försök`);
                }
            }
        }
        throw new Error("Oväntat fel i scrapeDivision");
    }
    async extractHorsesFromPage(page) {
        ...
        for (const selector of rowSelectors) {
            try {
                ...
                if (rows.length > 0) {
                    console.log(`Hittade ${rows.length} hästar med selector: ${selector}`);
        ...
        if (horses.length === 0) {
            ...
            const allRows = await page.$$('tr, .row, [class*="horse"], [class*="start"], div, li');
            horses = await this.extractHorsesFromRows(allRows);
        }
        return horses;
    }
    async extractHorsesFromRows(rows) {
        const horses = [];
     ...
     */
    async extractSpeltipsFromPage(page) {
        try {
            console.log("Extraherar speltips-data...");
            const speltips = {};
            // Sök efter Rank A, B, C
            const rankElements = await page.$$('div:has-text("Rank"), div:has-text("A:"), div:has-text("B:"), div:has-text("C:")');
            ...
            const spetsanalysElements = await page.$$('div:has-text("Spetsanalys"), div:has-text("spetsanalys")');
            ...
            // Sök efter källa
            const sourceElements = await page.$$('div:has-text("Källa:"), div:has-text("Source:")');
            ...
            const tipElements = await page.$$('div:has-text("Detaljerade tips"), div:has-text("Tips")');
            const detailedTips = [];
    ...
    async extractHorseFromRow(row, rowIndex) {
        try {
            ...
            if (lines.length >= 6) {
                ...
                if (horseNumber && horseName && driver && trainer) {
                    return {
                        ...
                        odds: odds || undefined,
                        shoes: this.normalizeShoeText(shoes) || undefined,
                    };
                }
            }
            ...
            const numberText = await this.extractTextWithFallback(row, numberSelectors);
            ...
            console.warn(`Fel vid extrahering av hästdata från rad ${rowIndex}:`, error);
            return null;
        }
    }
    async extractTextWithFallback(row, selectors) {
    ...
    parseHorseNumber(text) {
        if (!text)
            return null;
        const match = text.match(/\d+/);
        return match ? parseInt(match[0], 10) : null;
    }
    normalizeShoeText(text) {
        if (!text)
            return null;
        const normalized = text.toLowerCase().trim();
        // Filtrera bort fel data som inte är sko-information
        if (normalized.includes("distans") ||
        ...
        if (normalized.includes("barfota fram") &&
            normalized.includes("skor bak")) {
            return "c̶c";
        }
        if (normalized.includes("skor fram") &&
            normalized.includes("barfota bak")) {
            return "cc̶";
        }
        if (normalized.includes("barfota runt om") ||
            normalized.includes("barfota överallt")) {
            return "c̶c̶";
        }
        if (normalized.includes("skor runt om") ||
...
async function main() {
    ...
    try {
        ...
        // Scrapa data
        const v75Data = await scraper.scrapeV75(validatedArgs.date, validatedArgs.track, validatedArgs.bane);
        // Validera data
        const validatedData = V75Data.parse(v75Data);
        // Skriv till Markdown
        markdownWriter.writeV75Data(validatedData);
        console.log("Scraping slutfört!");
        console.log(`Totalt ${validatedData.divisions.reduce((sum, div) => sum + div.horses.length, 0)} hästar scrapade`);
    }
    catch (error) {
