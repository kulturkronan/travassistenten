...
export async function scrapeV75WithCookies(cookies, userAgent) {
    let browser = null;
    try {
        ...
        const v75Data = await page.evaluate(() => {
            ...
            console.log(`🐎 Hittade ${horses.length} potentiella hästnamn:`, horses.slice(0, 10));
            ...
            console.log(`❌ Hittade ${scratchedMatches ? scratchedMatches.length : 0} strukna hästar`);
            // Sök efter odds och procent
            const oddsPattern = /\d+[,.]\d+/g;
            const oddsMatches = bodyText.match(oddsPattern);
            console.log(`💰 Hittade ${oddsMatches ? oddsMatches.length : 0} odds/procent`);
...
//# sourceMappingURL=v75-with-cookies.js.map